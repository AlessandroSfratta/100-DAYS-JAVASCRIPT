...
const populateQuestion = () => {
    questions.forEach(question => {
        ...
        question.answers.forEach( answer => {
            ...
            sourceLink.textContent = "Unspash"
...
const handleClick = (questionId, chosenAnswer) => {
    if(unansweredQuestion.includes(questionId))
    chosenAnswers.push(chosenAnswer)
    const itemToRemove = unansweredQuestion.indexOf(questionId)

  if (itemToRemove > -1) {
    unansweredQuestion.splice(itemToRemove, 1)
  }
...
disableQuestionBlock(questionId, chosenAnswer)
  const lowestQuestionId = Math.min(...unansweredQuestion)
